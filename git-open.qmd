...
"공개(open)"의 반대는 "폐쇄(closed)"가 아니라 "망가짐(broken)"이다.<br>
The opposite of "open" isn't "closed". The opposite of "open" is "broken".<br>
-- John Wilbanks
:::
...
* 적당한 분석 결과가 생성되면, 논문을 작성해서 제출한다.
데이터를 논문에 포함할 수도 있다. (점점 많은 저널이 데이터를 요구한다.) 하지만 프로그램 코드는 포함하지 않을 가능성이 크다.
* 시간이 흐른다.
* 저널에서 연구원 분야의 익명의 소수 심사자로부터 받은 검토(review) 결과를 보낸다. 검토 결과를 충족하도록 논문을 수정한다.
수정하는 동안 이전에 작성한 프로그램과 스크립트를 변경해서 다시 제출한다.
* 더 많은 시간이 흐른다.
* 최종적으로 논문이 출판된다.
논문에 데이터의 온라인 사본 링크를 포함할 수도 있다.
하지만 논문은 유료 장벽(paywall)에 막혀있어서 개인이나 기관 접근 권한을 가진 사람만이 논문을 읽을 수 있다.

하지만 점점 더 많은 과학자들이 진행하는 연구 프로세스는 다음과 같다.

* 과학자가 수집한 데이터는 수집되는 즉시 [figshare](http://figshare.com/)나 [Zenodo](http://zenodo.org)와 같은 공개 접근 저장소에 저장되고,
  [디지털 객체 식별자(Digital Object Identifier, DOI)](https://en.wikipedia.org/wiki/Digital_object_identifier)가 부여된다.
  또는 데이터를 미리 게시하고 [Dryad](http://datadryad.org/)에 저장한다.
* 과학자가 작업물을 보관할 GitHub 저장소를 생성한다.
* 분석 작업을 수행하면서 스크립트의 변경 사항을 (아마도 몇몇 산출물도 포함해서) 저장소에 푸시한다.
  논문을 위한 저장소를 다목적으로 사용하며, 이 저장소가 다른 동료 과학자와 협업하는 허브가 된다.
* 논문 상태에 만족할 정도로 진행되면,
  [arXiv](http://arxiv.org/) 혹은 다른 사전 출판 서비스에 게시하고 다른 동료 과학자를 초대해서 피드백을 받는다.
* 피드백에 기초해서 저널에 논문을 최종 제출하기 전에 몇 번의 수정 사항을 게시할 수도 있다.
* 출판된 논문은 사전 출판 논문, 코드, 그리고 데이터 저장소의 링크를 포함한다.
  이를 통해 다른 과학자가 해당 연구를 시작점으로 삼아 연구를 쉽게 연결해서 수행할 수 있다.
...
* 누가 언제 무엇을 했는지를 포함해서 작업에 대한 개념적 단계가 문서화된다.
  모든 단계는 (커밋 ID) 식별자로 도장이 찍힌다. 식별자는 의도와 목적을 가지며 중복되지 않고 유일성을 갖는다.
...
2~3분 정도 인터넷을 검색하고 앞에서 언급된 데이터 저장소를 조사해 본다. 예를 들어,
...
`.md5`나 `.psd`와 같은 대용량 데이터나 이미지 파일은 오픈 소스 확장 도구인 Git Large File Storage를 사용하여 GitHub 저장소 내에서 추적할 수 있다. 자동으로 대용량 파일의 내용을 원격 서버에 업로드하고 GitHub 저장소 내의 파일을 텍스트 포인터로 대체한다.
...
먼저, 짝을 이룬다. 한 사람이 "소유자"(연습을 시작하는 데 사용될 GitHub 저장소 주인)가 되고,
...
![GitHub에 협업자(Collaborators) 추가](images/git/github-add-collaborators.png){#fig-collab-add}
...
![저장소 클론한 후 모습](images/git/github-collaboration.svg){#fig-collab-clone-after}

앞서 작업했던 것과 정확히 동일한 방식으로
...
소유자 로컬 컴퓨터로 GitHub 원본 저장소의 변경 사항을 다운로드하려면
소유자는 다음과 같이 입력한다.
...
저장소를 복제(clone)할 때 Git이 자동으로 `origin` 이름을 붙여주기 때문에, `origin`이라는 원격 저장소를 따로 생성할 필요가 없다는 점에 주목한다. (이것이 앞에서 원격 설정을 수작업으로 할 때 `origin` 이름을 사용한 것이 현명한 선택이었던 이유다.) 
...
이제 로컬 저장소는 `origin`이라고 불리는 하나의 "원격 저장소"를 가지고 있었다. 원격 저장소는 다른 곳에 호스팅되어 있는 저장소의 복사본으로, 
푸시(push)하고 풀(pull)할 수 있는 곳이며, 하나의 원격 저장소로만 작업해야 할 이유는 없다. 예를 들어, 대규모 프로젝트에서는 자신의 GitHub 계정에 자신만의 복사본을 가질 수도 있고(이를 `origin`이라고 부를 수 있음), 또한 메인 "상위" 프로젝트 저장소(예제에서는 이를 `upstream`이라고 부르겠음)를 가질 수 있다. 
...
* `git remote rename [oldname] [newname]`: 원격 저장소의 로컬 별칭, 즉 이름을 변경한다. 예를 들어, 이를 사용하여 `upstream`을 `fred`로 변경할 수 있다.

:::

GitHub에서 협력자의 변경 사항을 다운로드하기 위해 
이제 소유자는 다음을 입력한다.
...
명령줄에서 협력자는 로컬 저장소에 원격 저장소 변경 사항을 
`git fetch origin master` 명령어를 사용해서 가져올 수 있다. 
하지만 그 자체로 병합(merge)되는 것은 아니다.
`git diff master origin/main` 명령어를 실행해서
협력자는 터미널에 변경 사항을 확인할 수 있다.

GitHub에서도 협력자는 포크된 저장소로 가서 "This branch is 1 commit behind Our-Repository:master." 메시지를 볼 수 있다.
`Compare` 아이콘과 링크가 걸려 있다. `Compare` 페이지에서
협력자는 base fork를 본인 저장소로 변경하고 나서 "compare across forks" 위에
링크를 클릭한다. 마지막으로 `head fork`를 주 저장소로 변경한다.
...
소스 코드, 원고, 다른 창의적 저작물을 갖는 저장소가 공개될 때는
저장소 기반 디렉터리에 `LICENSE` 혹은 `LICENSE.txt` 파일을 포함해서 콘텐츠가 어떤 라이선스로 이용 가능한지를 명확히 기술해야 한다.
...
독점적 라이선스와 대조적으로, [Open Source Initiative](http://opensource.org/)에서 공인된
...
GPL은 다른 대부분의 공개 소스 라이선스와 다른데,
[전염성이 있는(infective)](http://swcarpentry.github.io/git-novice/reference.html#infective) 특징이 있다. 
코드의 수정된 버전을 배포하는 누구나 혹은 GPL 코드를 포함한 어느 것이든지 *자신의* 코드도 동일하게 자유로이 공개 가능하게 만들어야 한다.

흔히 사용되는 라이선스를 선택하는 것이 기여자나 사용자의 삶을 편하게 한다.
왜냐하면 기여자나 사용자 모두 해당 라이선스에 친숙해서 사용할 때 상당한 양의 전문 용어를
꼼꼼히 살펴볼 필요가 없기 때문이다.
...
좋은 선택이 될 수 있는 라이선스 목록을 유지 관리하고 있다.
...
만약 저장소 콘텐츠가 소프트웨어가 아닌 데이터, 창의적 저작물(매뉴얼, 기술 보고서, 원고) 같은 연구 제품이 포함되면,
소프트웨어를 위해 설계된 라이선스 대부분은 적합하지 *않다*.

* **데이터:** 대부분 국가 사법권에서 데이터 유형 대부분은 자연에 대한 사실로 간주된다.
  그러므로 저작권 보호를 받을 자격이 없다. (단, 아마도 사진과 의료 영상 정보 등은 예외)
  따라서 저작자 표시로 사회적 혹은 학자적 기대치를 알리려고 저작권을 정의로 주장하는 방식으로 라이선스를 사용하는 것은 단지 법적으로 혼탁한 상황만 조장할 뿐이다.
  [크리에이티브 커먼즈 제로(Creative Commons Zero, CC0)](https://creativecommons.org/publicdomain/zero/1.0/) 처럼 
공중 도메인 권리 포기를 지지하는법적 표시를 분명히 하는 것이 더 낫다. 
...
출처 표시 ([CC-BY](http://creativecommons.org/licenses/by/4.0/))와 
동일조건변경허락([CC-BY-SA](http://creativecommons.org/licenses/by-sa/4.0/)) 라이선스만이 "[오픈](http://opendefinition.org/) 라이선스"로 간주된다.
...
본인 프로젝트를 참조하는 방법을 기술하는 데 `CITATION` 혹은 `CITATION.txt` 파일을 포함할 수도 있다. 소프트웨어 카펜트리 사례는 다음과 같다:
...
세 번째 선택지는 [GitHub](http://github.com), [BitBucket](http://bitbucket.org), 혹은
...
예를 들어 소프트웨어 카펜트리는 [GitHub](https://github.com/swcarpentry)에 있어서
[해당 페이지에 대한 소스 코드](#git-hosting)를 찾아볼 수 있다.
...
여전히 CI가 자가 구축한 호스팅 환경에서도 이용 가능하지만
온라인 서비스 사용과 연계되면 초기 설정과 유지 보수 업무를 줄일 수 있다. 
...
제도 혁파를 통해서 더 공개된 과학을 지지하도록 좀 더 
앞서 나가는 데 근본적인 동기에 관해 질의하는 것이 더 생산적일 수 있다.
...
본인 논문, 데이터, 소프트웨어를 공유하려면 이용 가능한 저장소가 
소속 기관에 갖추어져 있는가?
소속 기관 저장소는 [arXiV](https://arxiv.org/), [figshare](https://figshare.com/), [GitHub](https://github.com/), [GitLab](https://about.gitlab.com/)과 
