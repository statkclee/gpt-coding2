...
추가한다면, 문자열이 매칭될 때, 무시된다. 하지만, `str_match()`을 사용할
...
`str_match()`에 원하는 부동 소수점 숫자를 표현하는 정규 표현식 부분에
괄호를 추가한다.
...
숫자)를 추출하고자 한다면 다음과 같이 프로그램을 작성할 수 있다.
...
점을 의미한다. 꺾쇠 괄호 외부에 점은 "와일드 카드(wild-card)" 문자이고 임의의 문자와 매칭한다. 꺾쇠 괄호 내부에서 점은 점일 뿐이다.
