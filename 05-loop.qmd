...
다음은 R 내장 `min()` 함수의 간략 버전이다. `getAnywhere(min)`, `.Primitive("min")`을 입력해도 원소스코드를 볼 수는 없다. R 콘솔에서 `min` 함수 내부코드를 바로 살펴볼 수 없는데 이유는 `C`로 작성되었기 때문이다. `names(methods:::.BasicFunsList)` 명령어를 통해 `.Primitive()` 함수를 파악할 수 있다.
