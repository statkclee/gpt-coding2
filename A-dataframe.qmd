...
해설: 리스트-칼럼 자료구조는 복잡한 데이터 구조를 다루는 데 유용하다. 단순한 숫자형 벡터는 데이터프레임의 열로 직접 저장하는 것이 더 적합하다.
...
::: {.content-visible when-format="revealjs"}
   리스트-칼럼 자료구조는 데이터프레임 내에 리스트를 포함할 수 있어 복잡한 데이터 구조를 효과적으로 다룰 수 있다는 장점이 있다. 이를 통해 정규표현식 처리 결과, 웹 API 데이터, 그룹별 연산 결과 등을 데이터프레임에 직접 저장할 수 있다. 
   
   리스트-칼럼을 활용하여 데이터를 처리하고 분석하는 과정에서는 티블(tibble) 형태의 데이터프레임을 사용하면 데이터 탐색과 조작이 수월해진다. 예를 들어, glimpse() 함수로 데이터 구조를 파악하고, []나 $ 연산자로 원소에 접근하며, mutate()나 summarise() 함수로 연산을 수행할 수 있다. 또한 unnest() 함수를 사용하여 리스트-칼럼을 개별 행으로 풀어내어 분석에 용이한 형태로 변환할 수 있다.
:::
